...
import {MapPin, Thermometer, Droplets, Wind, TriangleAlert} from 'lucide-react';
...
export default function InteractiveMap({ selectedForest, activeView }: InteractiveMapProps) {
  ...
  // Get selected forest data
  const selectedForestData = selectedForest
    ? forestsData.forests.find(forest => forest.id === selectedForest)
    : null;
  ...
  return (
    <div className="relative h-full w-full">
      ...
      {/* Stat Cards Overlay */}
      {selectedForestData && selectedForestData.stats && (
        <StatCards
          forestName={selectedForestData.name}
          stats={selectedForestData.stats}
        />
      )}
    </div>
  );
